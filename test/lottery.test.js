...
contract('Lottery', ([alice, bob, carol, cal, dev, minter]) => {
    ...
    it('test drawing', async () => {
        await this.lottery.multiClaim('5', {from: carol });
        await this.lottery.multiClaim('1', {from: alice });
        await this.lottery.multiClaim('2', {from: bob });
        await this.lottery.multiClaim('2', {from: bob });
        await this.lottery.multiClaim('20', {from: cal });
        await expectRevert(this.lottery.multiClaim('1', {from: alice }), 'exceed tickets amount');
        await expectRevert(this.lottery.multiClaim('1', {from: dev }), 'exceed tickets amount');
        ...
        console.log(id1, id2)
        await this.usdt.approve(this.lottery.address, '2000', { from: cal });
        await this.usdt.approve(this.lottery.address, '2000', { from: carol });
